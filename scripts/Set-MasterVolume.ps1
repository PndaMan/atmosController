...
# Sets the system master volume level using Windows Core Audio API
    ...
    public class AudioUtilities
    {
        public static void SetMasterVolume(float volume)
        {
            ...
            try
            {
                ...
                endpointVolume.SetMasterVolumeLevelScalar(volume / 100.0f, ref guid);
            }
            finally
            {
...
try {
    [CoreAudio.AudioUtilities]::SetMasterVolume($Volume)

    $result = @{
        volume = $Volume
        success = $true
        message = "Master volume set successfully"
    }
